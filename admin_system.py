...
def init_admin_tables():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS user_overrides (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def grant_user_access(user_email, plan='premium', duration_days=None, reason=''):
    ...
    c.execute('''INSERT INTO user_overrides (user_id, override_type, reason, granted_by, expires_at)
                 VALUES (?, ?, ?, ?, ?)''',
...
def create_discount_code(code, discount_percent=None, plan_override=None, uses=None, days_valid=None):
    ...
    try:
        c.execute('''INSERT INTO discount_codes (code, discount_percent, plan_override, uses_remaining, expires_at)
                     VALUES (?, ?, ?, ?, ?)''',
    ...
    with tab1:
        ...
        with st.form("grant_access_form"):
            ...
            if st.form_submit_button("Grant Access"):
                if user_email:
                    success, message = grant_user_access(user_email, plan,
                                                         duration if duration > 0 else None, reason)
                    if success:
                        st.success(message)
                    else:
                        st.error(message)
        ...
        with col1:
            ...
            with st.form("discount_form"):
                ...
                if st.form_submit_button("Create Discount Code"):
                    if code:
                        success, message = create_discount_code(code, discount, None,
                                                                uses if uses > 0 else None,
        ...
        with col2:
            ...
            with st.form("free_access_form"):
                ...
                if st.form_submit_button("Create Free Access Code"):
                    if code2:
                        success, message = create_discount_code(code2, None, plan2,
                                                                uses2 if uses2 > 0 else None,
                                                                days2 if days2 > 0 else None)
                        if success:
                            st.success(message)
